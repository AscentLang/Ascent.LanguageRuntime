...
namespace KTrie
{
    public sealed class Trie : ICollection<string>, IReadOnlyCollection<string>
    {
        ...
        public void Clear()
        {
            _root.Children = Array.Empty<CharTrieNode>();
            Count = 0;
        }
        ...
        internal (CharTrieNode? existingTerminalNode, CharTrieNode parent) AddNodesFromUpToBottom(
        ...
        internal void AddTerminalNode(CharTrieNode parent, CharTrieNode? existingNode, CharTrieNode newTerminalNode,
            string word)
        {
        ...
        {
            Queue<(CharTrieNode node, int index)> queue = new();
            ...
            while (queue.Count > 0)
            {
                ...
                if (index == pattern.Count - 1)
                {
                    ...
                    {
                        var n = GetChildNode(node, pattern[index].C);
        ...
        private Stack<CharTrieNode> GetNodesForRemoval(string prefix)
        {
            ...
            Stack<CharTrieNode> nodesUpToBottom = new();
            ...
            return current.IsTerminal ? nodesUpToBottom : new();
        }
